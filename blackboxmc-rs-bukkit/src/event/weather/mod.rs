...
pub struct LightningStrikeEventCause<'mc>(pub(crate) blackboxmc_general::SharedJNIEnv<'mc>, pub(crate) jni::objects::JObject<'mc>);
...
if !valid {
	...
	pub fn new_with_world(jni: blackboxmc_general::SharedJNIEnv<'mc>,arg0: impl Into<&'mc crate::World<'mc>>,arg1: std::option::Option<impl Into<&'mc crate::entity::LightningStrike<'mc>>>,arg2: std::option::Option<impl Into<&'mc crate::event::weather::LightningStrikeEventCause<'mc>>>) 
...
{let res = self.jni_ref().call_method(&self.jni_object(),"getCause","()Lorg/bukkit/event/weather/LightningStrikeEvent$Cause;",&[]);
let res = 
self.jni_ref().translate_error(res)?;
...
if !valid {
	...
	pub fn new(jni: blackboxmc_general::SharedJNIEnv<'mc>,arg0: impl Into<&'mc crate::World<'mc>>,arg1: bool) 
...
crate::event::HandlerList::from_raw(&jni,obj
	...
	pub fn to_thunder_state(&mut self) 
-> Result<bool, Box<dyn std::error::Error>>

{let res = self.jni_ref().call_method(&self.jni_object(),"toThunderState","()Z",&[]);
let res = 
self.jni_ref().translate_error(res)?;
Ok(res.z().unwrap())}
	pub fn world(&mut self) 
...
crate::event::HandlerList::from_raw(&jni,obj
	...
	pub fn to_weather_state(&mut self) 
-> Result<bool, Box<dyn std::error::Error>>

{let res = self.jni_ref().call_method(&self.jni_object(),"toWeatherState","()Z",&[]);
let res = 
self.jni_ref().translate_error(res)?;
Ok(res.z().unwrap())}
	pub fn world(&mut self) 
