...
impl<'mc> RaidFinishEvent<'mc> {
    ...
    pub fn winners(&self) -> Result<Vec<jni::objects::JObject<'mc>>, Box<dyn std::error::Error>> {
...
impl<'mc> RaidSpawnWaveEvent<'mc> {
    ...
    pub fn raiders(&self) -> Result<Vec<jni::objects::JObject<'mc>>, Box<dyn std::error::Error>> {
        let sig = String::from("()LVec;");
        let res =
            self.jni_ref()
