...
let val_4 = jni::objects::JObject::from(self.jni_ref().new_string(arg3.unwrap().into()).unwrap());
...
let res = self.jni_ref().call_method(&self.jni_object(),"getBanEntry","(Ljava/lang/Object;)Lorg/bukkit/BanEntry;",&[jni::objects::JValueGen::from(&val_1)]);
let res = 
self.jni_ref().translate_error(res)?;
crate::BanEntry::from_raw(&self.jni_ref(),unsafe { jni::objects::JObject::from_raw(res.l()?.clone()) }
)}
...
{let val_1 = arg0.unwrap();
let res = self.jni_ref().call_method(&self.jni_object(),"isBanned","(Ljava/lang/Object;)Z",&[jni::objects::JValueGen::from(&val_1)]);
let res = 
self.jni_ref().translate_error(res)?;
Ok(res.z().unwrap())}
#[deprecated]
...
let res = self.jni_ref().call_method(&self.jni_object(),"pardon","(Ljava/lang/String;)V",&[jni::objects::JValueGen::from(&val_1)]);
self.jni_ref().translate_error(res)?;
Ok(())}
}
