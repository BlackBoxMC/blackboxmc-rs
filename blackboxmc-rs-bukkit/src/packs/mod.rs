...
impl<'mc> DataPack<'mc> {
    ...
    /// a list of known pack versions, see the
    /// <a href="https://minecraft.wiki/w/Data_pack#Pack_format">Minecraft Wiki</a>.
    pub fn min_supported_pack_format(&self) -> Result<i32, Box<dyn std::error::Error>> {
        let sig = String::from("()I");
    ...
    /// a list of known pack versions, see the
    /// <a href="https://minecraft.wiki/w/Data_pack#Pack_format">Minecraft Wiki</a>.
    pub fn max_supported_pack_format(&self) -> Result<i32, Box<dyn std::error::Error>> {
        let sig = String::from("()I");
        let res = self.jni_ref().call_method(
            &self.jni_object(),
...
impl<'mc> std::ops::Deref for DataPackCompatibility<'mc> {
    type Target = DataPackCompatibilityStruct<'mc>;
    fn deref(&self) -> &<DataPackCompatibility<'mc> as std::ops::Deref>::Target {
        match self {
            DataPackCompatibility::New { inner } => inner,
            DataPackCompatibility::Old { inner } => inner,
            DataPackCompatibility::Compatible { inner } => inner,
        }
    }
}
...
impl<'mc> std::ops::Deref for DataPackSource<'mc> {
    type Target = DataPackSourceStruct<'mc>;
    fn deref(&self) -> &<DataPackSource<'mc> as std::ops::Deref>::Target {
        match self {
            DataPackSource::Default { inner } => inner,
            DataPackSource::BuiltIn { inner } => inner,
            DataPackSource::Feature { inner } => inner,
            DataPackSource::World { inner } => inner,
            DataPackSource::Server { inner } => inner,
        }
    }
}
...
impl<'mc> DataPackManager<'mc> {
    ...
    pub fn get_data_pack(
        &self,
        data_pack_key: impl Into<crate::NamespacedKey<'mc>>,
        ...
        let val_1 = jni::objects::JValueGen::Object(unsafe {
            jni::objects::JObject::from_raw(data_pack_key.into().jni_object().clone())
        });
        let res = self.jni_ref().call_method(
            &self.jni_object(),
            "getDataPack",
        ...
        Ok(Some(crate::packs::DataPack::from_raw(
            &self.jni_ref(),
...
impl<'mc> JNIInstantiatable<'mc> for ResourcePack<'mc> {
    fn from_raw(
        ...
        if obj.is_null() {
            return Err(eyre::eyre!("Tried to instantiate ResourcePack from null object.").into());
        }
...
impl<'mc> ResourcePack<'mc> {
    ...
    pub fn url(&self) -> Result<String, Box<dyn std::error::Error>> {
        ...
        let res = self
            .jni_ref()
            .call_method(&self.jni_object(), "getUrl", sig.as_str(), vec![]);
        let res = self.jni_ref().translate_error(res)?;
        Ok(self
            .jni_ref()
            .get_string(unsafe { &jni::objects::JString::from_raw(res.as_jni().l) })?
    ...
    pub fn hash(&self) -> Result<Option<String>, Box<dyn std::error::Error>> {
        let sig = String::from("()Ljava/lang/String;");
        let res = self
            .jni_ref()
            .call_method(&self.jni_object(), "getHash", sig.as_str(), vec![]);
        let res = self.jni_ref().translate_error(res)?;
        if unsafe { jni::objects::JObject::from_raw(res.as_jni().l) }.is_null() {
            return Ok(None);
        }
    ...
    pub fn prompt(&self) -> Result<Option<String>, Box<dyn std::error::Error>> {
        let sig = String::from("()Ljava/lang/String;");
        let res = self
            .jni_ref()
            .call_method(&self.jni_object(), "getPrompt", sig.as_str(), vec![]);
        let res = self.jni_ref().translate_error(res)?;
        if unsafe { jni::objects::JObject::from_raw(res.as_jni().l) }.is_null() {
            return Ok(None);
        }
