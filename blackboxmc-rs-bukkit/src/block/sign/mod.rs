...
use color_eyre::eyre::Result;/*org/bukkit/block/sign/mod.rs*/
...
impl<'mc> std::fmt::Display for Side<'mc> {
   fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
       match self {
        ...
        impl<'mc> SideTrait<'mc> for Side<'mc> {}
        
        pub trait SideTrait<'mc>: blackboxmc_general::JNIRaw<'mc> + blackboxmc_general::JNIInstantiatable<'mc>  {
            fn value_of(
...
"FRONT" => Ok(Side::Front { inner: SideStruct::from_raw(env,obj)?}),
"BACK" => Ok(Side::Back { inner: SideStruct::from_raw(env,obj)?}),
...
#[repr(C)]
pub struct SideStruct<'mc>(pub(crate) blackboxmc_general::SharedJNIEnv<'mc>, pub(crate) jni::objects::JObject<'mc>);
        ...
        fn from_raw(
            ...
            if obj.is_null() {
                return Err(eyre::eyre!(
                    "Tried to instantiate Side from null object.")
                .into());
            }
            ...
            if !valid {
                ...
                let variant_str = env
                    ...
                    .to_string();
                match variant_str.as_str() {
                    "FRONT" => Ok(Side::Front { inner: SideStruct::from_raw(env,obj)?}),"BACK" => Ok(Side::Back { inner: SideStruct::from_raw(env,obj)?}),_ => Err(eyre::eyre!("String gaven for variant was invalid").into())}
            }
        }
    }
...
impl<'mc> JNIInstantiatable<'mc> for SideStruct<'mc> {
        fn from_raw(
            ...
            if obj.is_null() {
                return Err(eyre::eyre!(
                    "Tried to instantiate SideStruct from null object.")
                .into());
            }
            let (valid, name) = env.validate_name(&obj, "org/bukkit/block/sign/Side")?;
            if !valid {
...
let cls = jni.find_class("org/bukkit/block/sign/Side"); let cls = jni.translate_error_with_class(cls)?;
...
#[repr(C)]
pub struct SignSide<'mc>(pub(crate) blackboxmc_general::SharedJNIEnv<'mc>, pub(crate) jni::objects::JObject<'mc>);
...
impl<'mc> JNIInstantiatable<'mc> for SignSide<'mc> {
        fn from_raw(
            ...
            if obj.is_null() {
                return Err(eyre::eyre!(
                    "Tried to instantiate SignSide from null object.")
                .into());
            }
...
impl<'mc> SignSideTrait<'mc> for SignSide<'mc> {}
pub trait SignSideTrait<'mc>: blackboxmc_general::JNIRaw<'mc> + blackboxmc_general::JNIInstantiatable<'mc> {
...
crate::material::Colorable::from_raw(&self.jni_ref(), self.jni_object()).expect("Error converting SignSide into crate::material::Colorable")

   }
}
impl<'mc> crate::material::ColorableTrait<'mc> for SignSide<'mc> {}
