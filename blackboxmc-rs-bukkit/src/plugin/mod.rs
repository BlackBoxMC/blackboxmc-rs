...
impl<'mc> PluginDescriptionFile<'mc> {
	pub fn new_with_input_stream(jni: &blackboxmc_general::SharedJNIEnv<'mc>,arg0: jni::objects::JObject<'mc>) 
...
"UTF8" => Ok(Flags::Utf8 { inner: FlagsStruct::from_raw(env,obj)?}),
...
#[repr(C)]
pub struct FlagsStruct<'mc>(pub(crate) blackboxmc_general::SharedJNIEnv<'mc>, pub(crate) jni::objects::JObject<'mc>);
        ...
        fn from_raw(
            ...
            if !valid {
                ...
                let variant_str = env
                    ...
                    .to_string();
                match variant_str.as_str() {
                    "UTF8" => Ok(Flags::Utf8 { inner: FlagsStruct::from_raw(env,obj)?}),_ => Err(eyre::eyre!("String gaven for variant was invalid").into())}
            }
        }
    }
...
impl<'mc> JNIInstantiatable<'mc> for FlagsStruct<'mc> {
        fn from_raw(
            ...
            if obj.is_null() {
                return Err(eyre::eyre!(
                    "Tried to instantiate FlagsStruct from null object.")
                .into());
            }
            let (valid, name) = env.validate_name(&obj, "org/bukkit/plugin/Flags")?;
            if !valid {
...
while iter.has_next()? {            let obj = iter.next()?;
new_vec.push(crate::plugin::RegisteredServiceProvider::from_raw(&self.jni_ref(),obj,)?);
};Ok(
new_vec
)}
...
"UTF8" => Ok(FlagsFlags::Utf8 { inner: FlagsFlagsStruct::from_raw(env,obj)?}),
...
#[repr(C)]
pub struct FlagsFlagsStruct<'mc>(pub(crate) blackboxmc_general::SharedJNIEnv<'mc>, pub(crate) jni::objects::JObject<'mc>);
        ...
        fn from_raw(
            ...
            if !valid {
                ...
                let variant_str = env
                    ...
                    .to_string();
                match variant_str.as_str() {
                    "UTF8" => Ok(FlagsFlags::Utf8 { inner: FlagsFlagsStruct::from_raw(env,obj)?}),_ => Err(eyre::eyre!("String gaven for variant was invalid").into())}
            }
        }
    }
...
impl<'mc> JNIInstantiatable<'mc> for FlagsFlagsStruct<'mc> {
        fn from_raw(
            ...
            if obj.is_null() {
                return Err(eyre::eyre!(
                    "Tried to instantiate FlagsFlagsStruct from null object.")
                .into());
            }
            let (valid, name) = env.validate_name(&obj, "org/bukkit/plugin/Flags$Flags")?;
            if !valid {
