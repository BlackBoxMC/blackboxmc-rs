...
use color_eyre::eyre::Result;/*org/bukkit/entity/memory/mod.rs*/

#[repr(C)]
pub struct MemoryKey<'mc>(pub(crate) blackboxmc_general::SharedJNIEnv<'mc>, pub(crate) jni::objects::JObject<'mc>);
        ...
        fn from_raw(
            ...
            if obj.is_null() {
                return Err(eyre::eyre!(
                    "Tried to instantiate MemoryKey from null object.")
                .into());
            }
...
impl<'mc> MemoryKeyTrait<'mc> for MemoryKey<'mc> {}
pub trait MemoryKeyTrait<'mc>: blackboxmc_general::JNIRaw<'mc> + blackboxmc_general::JNIInstantiatable<'mc> {
...
/// Gets the class of values associated with this memory.
	fn memory_class(&self) 
-> Result<jni::objects::JClass<'mc>, Box<dyn std::error::Error>>

{let sig = String::from("()Ljava/lang/Class;");
let res = self.jni_ref().call_method(&self.jni_object(),"getMemoryClass",sig.as_str(),vec![]);
let res = 
self.jni_ref().translate_error(res)?;
Ok(
...
let cls = jni.find_class("org/bukkit/entity/memory/MemoryKey"); let cls = jni.translate_error_with_class(cls)?;
...
crate::Keyed::from_raw(&self.jni_ref(), self.jni_object()).expect("Error converting MemoryKey into crate::Keyed")

   }
}
impl<'mc> crate::KeyedTrait<'mc> for MemoryKey<'mc> {}
